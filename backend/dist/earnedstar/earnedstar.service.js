...
var EarnedstarService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.EarnedstarService = void 0;
...
let EarnedstarService = EarnedstarService_1 = class EarnedstarService {
    constructor(postgres, email, sms, photos, fraud, merchantStats, planLimits, indexNow) {
        ...
        this.logger = new common_1.Logger(EarnedstarService_1.name);
        this.merchantSelectCols = `id::text, name, slug, logo_url, website_url, seo_title, seo_description,
    ...
    async getMerchantBySlug(slug) {
        if (this.useMock()) {
            if (slug === 'meridian-gear' || slug === MOCK_MERCHANT.slug)
                return MOCK_MERCHANT;
            return { ...MOCK_MERCHANT, slug, name: slug.replace(/-/g, ' ') };
        }
        const row = await this.postgres.queryOne(`SELECT ${this.merchantSelectCols}
       FROM businesses WHERE slug = $1 LIMIT 1`, [slug]);
        if (!row)
            throw new common_1.NotFoundException(`Merchant not found: ${slug}`);
        return row;
    }
    async listPublishedReviews(slug, limit = 50, offset = 0, filters = {}) {
        if (this.useMock()) {
    ...
    async listMerchantReviews(slug, limit = 100) {
        if (this.useMock()) {
            await this.getMerchantBySlug(slug);
            return MOCK_REVIEWS;
        }
    ...
    async queryReviews(businessId, limit, publishedOnly, offset = 0, filters = {}) {
        const statusFilter = publishedOnly ? `AND r.status = 'published'` : '';
        const conditions = [`r.business_id = $1::uuid ${statusFilter}`];
        ...
        if (filters.sort === 'highest')
            orderBy = 'r.rating_overall DESC, r.created_at DESC';
        if (filters.sort === 'lowest')
            orderBy = 'r.rating_overall ASC, r.created_at DESC';
        params.push(limit, offset);
        return this.postgres.queryMany(`SELECT r.id::text, r.business_id::text, r.customer_name, r.order_id,
    ...
    async getPublicProfileSummary(slug) {
        if (this.useMock()) {
            const merchant = await this.getMerchantBySlug(slug);
            return {
                merchant,
        ...
        const attrs = await this.postgres.queryOne(`SELECT
       ...
       FROM reviews
       WHERE business_id = $1::uuid AND status = 'published'`, [merchant.id]);
        return {
            merchant,
            ratingDistribution,
            attributeAverages: {
    ...
    async getRatingDistribution(businessId) {
        const rows = await this.postgres.queryMany(`SELECT FLOOR(rating_overall)::int AS stars, COUNT(*)::int AS count
       FROM reviews
       WHERE business_id = $1::uuid AND status = 'published'
       GROUP BY 1 ORDER BY stars DESC`, [businessId]);
    ...
    async getDashboardOverview(slug = 'meridian-gear') {
        ...
        const ratingDistribution = await this.getRatingDistribution(merchant.id);
        const invites = await this.postgres.queryOne(`SELECT
         COUNT(*) FILTER (WHERE status IN ('sent','opened','completed','expired'))::int AS sent,
         COUNT(*) FILTER (WHERE status = 'completed')::int AS completed
       FROM review_requests WHERE business_id = $1::uuid`, [merchant.id]);
        const inviteResponseRate = invites && invites.sent > 0
    ...
    async submitReview(dto) {
        ...
        const invitation = await this.postgres.queryOne(`SELECT rr.business_id::text, rr.order_id, b.plan
       FROM review_requests rr
       JOIN businesses b ON b.id = rr.business_id
       WHERE rr.token = $1 AND rr.status IN ('sent','opened') LIMIT 1`, [dto.token]);
        if (!invitation)
        ...
        if (dto.video_url && !this.planLimits.planAllowsVideo(invitation.plan)) {
            throw new common_1.ForbiddenException('Video reviews require Growth plan or higher');
        }
        const recentRows = await this.postgres.queryMany(`SELECT review_text FROM reviews
       WHERE business_id = $1::uuid AND review_text IS NOT NULL
       ORDER BY created_at DESC LIMIT 25`, [invitation.business_id]);
        const recentTexts = recentRows.map((r) => r.review_text ?? '').filter(Boolean);
        const verdict = this.fraud.scoreSubmission(dto, recentTexts);
        const row = await this.postgres.queryOne(`INSERT INTO reviews (
      ...
      RETURNING id::text`, [
        ...
        if (verdict.status === 'published') {
            ...
            const slugRow = await this.postgres.queryOne(`SELECT slug FROM businesses WHERE id = $1::uuid LIMIT 1`, [invitation.business_id]);
            if (slugRow?.slug)
                this.pingProfileSearch(slugRow.slug);
        }
        await this.postgres.query(`UPDATE review_requests SET status = 'completed', review_submitted_at = now() WHERE token = $1`, [dto.token]);
        return {
            ok: true,
            reviewId: row?.id,
    ...
    async sendInvitationForOwner(ownerId, slug, dto) {
        if (this.useMock()) {
    ...
    async sendInvitation(slug, dto) {
        ...
        if (channel === 'sms' && !dto.customer_phone?.trim()) {
            throw new common_1.BadRequestException('customer_phone is required for SMS invitations');
        }
        if (channel === 'email' && !dto.customer_email?.trim()) {
            throw new common_1.BadRequestException('customer_email is required for email invitations');
        }
        const scheduled = delayDays > 0 && channel !== 'link';
        const status = scheduled ? 'scheduled' : 'sent';
        const customerEmail = dto.customer_email?.trim() ||
            (dto.customer_phone
        ...
        const row = await this.postgres.queryOne(`INSERT INTO review_requests (
    ...
    async bulkSendInvitationsForOwner(ownerId, slug, dto) {
    ...
    async resendInvitationForOwner(ownerId, slug, invitationId) {
        if (this.useMock()) {
        ...
        const merchant = await this.getMerchantForOwner(ownerId);
        const row = await this.postgres.queryOne(`SELECT customer_email, customer_phone, customer_name, order_id, channel
       FROM review_requests
       WHERE id = $1::uuid AND business_id = $2::uuid
       LIMIT 1`, [invitationId, merchant.id]);
        if (!row)
    ...
    async dispatchInvitationDelivery(input) {
    ...
    async flushScheduledInvitations(businessId) {
        if (this.useMock() || !this.postgres.isConfigured())
            return;
        const due = await this.postgres.queryMany(`SELECT rr.id::text, rr.token, rr.customer_email, rr.customer_phone, rr.customer_name,
       ...
       LIMIT 25`, [businessId]);
        for (const row of due) {
            ...
            await this.postgres.query(`UPDATE review_requests SET status = 'sent', sent_at = now() WHERE id = $1::uuid`, [row.id]);
        }
    }
    async getInvitationByToken(token) {
        ...
        const row = await this.postgres.queryOne(`SELECT rr.token, rr.status, b.name AS merchant_name, b.slug AS merchant_slug, rr.order_id,
    ...
    async listInvitations(slug, limit = 50) {
        if (this.useMock()) {
            return [
                {
                    id: 'inv1',
        ...
        const merchant = await this.getMerchantBySlug(slug);
        await this.flushScheduledInvitations(merchant.id);
        return this.postgres.queryMany(`SELECT id::text, customer_email, customer_name, order_id, channel, status,
              COALESCE(sent_at, send_at, created_at)::text AS sent_at,
              opened_at::text AS opened_at, token
       FROM review_requests
       WHERE business_id = $1::uuid
       ORDER BY created_at DESC
       LIMIT $2`, [merchant.id, limit]);
    }
    getAggregateSchema(slug) {
        if (this.useMock()) {
    ...
    async getMerchantEmailPrefs(businessId) {
        if (this.useMock()) {
            return { fromName: undefined, subjectTemplate: undefined };
        }
        const row = await this.postgres.queryOne(`SELECT email_from_name, email_subject_template FROM businesses WHERE id = $1::uuid LIMIT 1`, [businessId]);
        return {
            fromName: row?.email_from_name ?? undefined,
            subjectTemplate: row?.email_subject_template ?? undefined,
        };
    }
    async provisionMerchant(dto) {
        if (this.useMock()) {
        ...
        const existing = await this.postgres.queryOne(`SELECT id::text, slug FROM businesses WHERE owner_id = $1::uuid LIMIT 1`, [dto.owner_id]);
        if (existing) {
            return { ok: true, merchantId: existing.id, slug: existing.slug, existing: true };
        }
        const row = await this.postgres.queryOne(`INSERT INTO businesses (owner_id, name, slug, plan, website_url)
    ...
    async getMerchantForOwner(ownerId) {
        if (this.useMock())
            return MOCK_MERCHANT;
        const row = await this.postgres.queryOne(`SELECT ${this.merchantSelectCols}
       FROM businesses WHERE owner_id = $1::uuid LIMIT 1`, [ownerId]);
        if (!row)
            throw new common_1.NotFoundException('No merchant linked to this account');
        return row;
    }
    async updateMerchantBilling(businessId, patch) {
        if (this.useMock())
            return { ok: true };
        await this.postgres.query(`UPDATE businesses SET plan = $2,
       ...
       WHERE id = $1::uuid`, [businessId, patch.plan, patch.authnet_subscription_id ?? null, patch.authnet_customer_profile_id ?? null]);
        return { ok: true };
    }
    async handleOrderFulfilled(dto) {
    ...
    async getMerchantByApiKey(apiKey) {
        if (this.useMock()) {
            if (apiKey === 'demo' || apiKey === 'YOUR_API_KEY')
                return MOCK_MERCHANT;
            return MOCK_MERCHANT;
        }
        const row = await this.postgres.queryOne(`SELECT id::text, name, slug, logo_url, website_url, seo_title, seo_description,
              plan, api_key, review_count, avg_rating::float8 AS avg_rating
       FROM businesses WHERE api_key = $1 LIMIT 1`, [apiKey]);
        if (!row)
            throw new common_1.NotFoundException('Invalid API key');
        return row;
    }
    async getPublicEmbedByApiKey(apiKey) {
    ...
    async uploadReviewPhoto(dto) {
        if (this.useMock()) {
            return { ok: true, url: `data:image/jpeg;base64,${dto.data_base64.slice(0, 32)}` };
        }
        const invitation = await this.postgres.queryOne(`SELECT business_id::text FROM review_requests
       WHERE token = $1 AND status IN ('sent','opened') LIMIT 1`, [dto.token]);
        if (!invitation)
            throw new common_1.NotFoundException('Invalid or expired invitation token');
    ...
    async moderateReviewForOwner(ownerId, reviewId, dto) {
        if (this.useMock()) {
            return { ok: true, reviewId, status: dto.status };
        }
        const merchant = await this.getMerchantForOwner(ownerId);
        const existing = await this.postgres.queryOne(`SELECT status FROM reviews WHERE id = $1::uuid AND business_id = $2::uuid LIMIT 1`, [reviewId, merchant.id]);
        if (!existing)
            throw new common_1.NotFoundException('Review not found');
        await this.postgres.query(`UPDATE reviews SET status = $3, published_at = CASE WHEN $3 = 'published' THEN now() ELSE published_at END
       WHERE id = $1::uuid AND business_id = $2::uuid`, [reviewId, merchant.id, dto.status]);
    ...
    async respondToReviewForOwner(ownerId, reviewId, dto) {
        const text = dto.business_response.trim();
        if (this.useMock()) {
            return { ok: true, reviewId, business_response: text };
        }
        const merchant = await this.getMerchantForOwner(ownerId);
        const existing = await this.postgres.queryOne(`SELECT r.id::text, r.customer_name, r.customer_email,
              p.name AS product_name
       FROM reviews r
       LEFT JOIN products p ON p.id = r.product_id
       WHERE r.id = $1::uuid AND r.business_id = $2::uuid
       LIMIT 1`, [reviewId, merchant.id]);
        if (!existing)
            throw new common_1.NotFoundException('Review not found');
        await this.postgres.query(`UPDATE reviews SET business_response = $3, response_at = now()
       WHERE id = $1::uuid AND business_id = $2::uuid`, [reviewId, merchant.id, text]);
    ...
    async getPublicWidgetBySlug(slug, max = 12) {
        const merchant = await this.getMerchantBySlug(slug);
    ...
    async getAnalyticsDashboard(slug = 'meridian-gear') {
        ...
        const inviteRows = await this.postgres.queryMany(`SELECT to_char(date_trunc('week', created_at), 'Mon DD') AS week,
              COUNT(*)::int AS sent,
              COUNT(*) FILTER (WHERE status = 'completed')::int AS completed
       FROM review_requests
       WHERE business_id = $1::uuid AND created_at >= now() - interval '8 weeks'
       GROUP BY date_trunc('week', created_at)
       ORDER BY date_trunc('week', created_at)`, [merchant.id]);
        const reviewRows = await this.postgres.queryMany(`SELECT to_char(date_trunc('week', created_at), 'Mon DD') AS week,
       ...
       GROUP BY date_trunc('week', created_at)
       ORDER BY date_trunc('week', created_at)`, [merchant.id]);
        const sentimentRow = await this.postgres.queryOne(`SELECT
       ...
       FROM reviews
       WHERE business_id = $1::uuid AND status = 'published'`, [merchant.id]);
        const totalSentiment = (sentimentRow?.positive ?? 0) + (sentimentRow?.neutral ?? 0) + (sentimentRow?.negative ?? 0) || 1;
        return {
    ...
    buildWidgetEmbedCode(apiKey, widgetType, config = {}) {
  ...
  data-max="${max}"></script>`;
    ...
    async listWidgetsForOwner(ownerId) {
        const merchant = await this.getMerchantForOwner(ownerId);
        return this.listWidgets(merchant.id, merchant.api_key);
    }
    async listWidgets(merchantId, apiKey) {
        if (this.useMock()) {
        ...
        const rows = await this.postgres.queryMany(`SELECT id::text, name, widget_type, config, embed_code, created_at::text
       FROM review_widgets WHERE business_id = $1::uuid ORDER BY created_at DESC`, [merchantId]);
        const key = apiKey ??
            (await this.postgres.queryOne(`SELECT api_key FROM businesses WHERE id = $1::uuid`, [merchantId]))?.api_key ??
            'YOUR_API_KEY';
        return rows.map((row) => ({
            ...row,
    ...
    async createWidgetForOwner(ownerId, dto) {
        if (this.useMock()) {
            const merchant = await this.getMerchantForOwner(ownerId);
            return {
                ok: true,
        ...
        const row = await this.postgres.queryOne(`INSERT INTO review_widgets (business_id, name, widget_type, config, embed_code)
       VALUES ($1::uuid, $2, $3, $4::jsonb, $5)
       RETURNING id::text`, [merchant.id, dto.name, dto.widget_type, JSON.stringify(dto.config ?? {}), embedCode]);
        return {
            ok: true,
            widget: {
                id: row?.id,
    ...
    async deleteWidgetForOwner(ownerId, widgetId) {
        if (this.useMock())
            return { ok: true };
        const merchant = await this.getMerchantForOwner(ownerId);
        await this.postgres.query(`DELETE FROM review_widgets WHERE id = $1::uuid AND business_id = $2::uuid`, [widgetId, merchant.id]);
        return { ok: true };
    }
    async getShopifyIntegration(ownerId) {
        if (this.useMock()) {
            return {
                connected: false,
                shop: null,
        ...
        const merchant = await this.getMerchantForOwner(ownerId);
        const row = await this.postgres.queryOne(`SELECT shopify_shop, shopify_status FROM businesses WHERE id = $1::uuid`, [merchant.id]);
    ...
    async connectShopifyForOwner(ownerId, shop) {
        const normalized = shop.trim().toLowerCase().replace(/^https?:\/\//, '').replace(/\/$/, '');
        if (!normalized.endsWith('.myshopify.com')) {
            throw new common_1.BadRequestException('Shop must be a valid *.myshopify.com domain');
        }
        if (this.useMock()) {
            return { ok: true, shop: normalized, status: 'pending' };
        }
        const merchant = await this.getMerchantForOwner(ownerId);
        await this.postgres.query(`UPDATE businesses SET shopify_shop = $2, shopify_status = 'pending' WHERE id = $1::uuid`, [merchant.id, normalized]);
        return { ok: true, shop: normalized, status: 'pending' };
    }
    async getGoogleSellerFeed(slug) {
        const merchant = await this.getMerchantBySlug(slug);
    ...
    async getTrustpilotExport(slug) {
        const merchant = await this.getMerchantBySlug(slug);
        this.planLimits.assertCanAccessSyndication(merchant.plan);
    ...
    async exportReviewsCsvForOwner(ownerId, slug) {
        const merchant = await this.getMerchantForOwner(ownerId);
        if (slug && slug !== merchant.slug)
            await this.getMerchantBySlug(slug);
        const limits = this.planLimits.limitsFor(merchant.plan);
        if (!limits.analytics) {
            throw new common_1.ForbiddenException('CSV export requires Growth plan or higher');
        }
    ...
    async getOnboardingStatus(ownerId) {
        if (this.useMock()) {
            return { completed: true, step: 5 };
        }
        const row = await this.postgres.queryOne(`SELECT onboarding_completed_at::text FROM businesses WHERE owner_id = $1::uuid LIMIT 1`, [ownerId]);
        return { completed: Boolean(row?.onboarding_completed_at), step: row?.onboarding_completed_at ? 5 : 1 };
    }
    async completeOnboarding(ownerId, dto) {
        if (this.useMock())
            return { ok: true, completed: true };
        const merchant = await this.getMerchantForOwner(ownerId);
        await this.postgres.query(`UPDATE businesses SET
    ...
    async updateMerchantProfile(ownerId, dto) {
        if (this.useMock())
            return { ok: true, merchant: MOCK_MERCHANT };
        const merchant = await this.getMerchantForOwner(ownerId);
        await this.postgres.query(`UPDATE businesses SET
        name = COALESCE($2, name),
        website_url = COALESCE($3, website_url),
    ...
    async listAgencyClients(ownerId) {
        const agency = await this.getMerchantForOwner(ownerId);
        if (agency.plan !== 'agency') {
            throw new common_1.ForbiddenException('Agency plan required');
        }
        if (this.useMock()) {
            return [
        ...
        return this.postgres.queryMany(`SELECT id::text, name, slug, review_count, avg_rating::float8 AS avg_rating, website_url, created_at::text
       FROM businesses WHERE parent_business_id = $1::uuid ORDER BY created_at DESC`, [agency.id]);
    }
    async createAgencyClient(ownerId, dto) {
        const agency = await this.getMerchantForOwner(ownerId);
        if (agency.plan !== 'agency') {
            throw new common_1.ForbiddenException('Agency plan required');
        }
        ...
        if (this.postgres.isConfigured()) {
            const row = await this.postgres.queryOne(`SELECT COUNT(*)::int AS count FROM businesses WHERE parent_business_id = $1::uuid`, [agency.id]);
            const max = limits.sub_accounts ?? 25;
            if ((row?.count ?? 0) >= max) {
                throw new common_1.ForbiddenException(`Agency client limit reached (${max})`);
            }
        }
        const slug = dto.slug?.trim() || this.slugify(dto.business_name);
        if (this.useMock()) {
        ...
        const row = await this.postgres.queryOne(`INSERT INTO businesses (owner_id, parent_business_id, name, slug, website_url, plan)
       VALUES ($1::uuid, $2::uuid, $3, $4, $5, 'growth')
       RETURNING id::text, slug`, [ownerId, agency.id, dto.business_name, slug, dto.website_url ?? `https://earnedstar.com/store/${slug}`]);
        return { ok: true, clientId: row?.id, slug: row?.slug, name: dto.business_name };
    }
    async listPublishedQa(slug) {
        if (this.useMock()) {
            return [
                {
                    id: 'qa1',
        ...
        return this.postgres.queryMany(`SELECT id::text, question, answer, asked_by, answered_at::text, created_at::text
       FROM qa_items
       WHERE business_id = $1::uuid AND published = true AND answer IS NOT NULL
       ORDER BY answered_at DESC NULLS LAST, created_at DESC
       LIMIT 50`, [merchant.id]);
    }
    async publicAskQa(slug, question, askedBy) {
        if (this.useMock()) {
            return { ok: true, id: `qa-${Date.now()}` };
        }
        const merchant = await this.getMerchantBySlug(slug);
        const row = await this.postgres.queryOne(`INSERT INTO qa_items (business_id, question, asked_by, published)
       VALUES ($1::uuid, $2, $3, false)
       RETURNING id::text`, [merchant.id, question, askedBy ?? 'Customer']);
        return { ok: true, id: row?.id };
    }
    async listQaForOwner(ownerId) {
        const merchant = await this.getMerchantForOwner(ownerId);
        ...
        return this.postgres.queryMany(`SELECT id::text, question, answer, published, ai_generated, asked_by,
              answered_at::text, created_at::text
       FROM qa_items
       WHERE business_id = $1::uuid
       ORDER BY created_at DESC`, [merchant.id]);
    }
    async createQaForOwner(ownerId, dto) {
        const merchant = await this.getMerchantForOwner(ownerId);
        this.planLimits.assertCanAccessQa(merchant.plan);
        ...
        const row = await this.postgres.queryOne(`INSERT INTO qa_items (business_id, question, answer, asked_by, published, answered_at)
       VALUES ($1::uuid, $2, $3, $4, $5, CASE WHEN $3 IS NOT NULL THEN now() ELSE NULL END)
       RETURNING id::text`, [merchant.id, dto.question.trim(), answer, dto.asked_by?.trim() ?? 'Merchant', published]);
        if (published)
            this.pingProfileSearch(merchant.slug);
        return { ok: true, id: row?.id, published };
    }
    async updateQaForOwner(ownerId, id, dto) {
        const merchant = await this.getMerchantForOwner(ownerId);
        this.planLimits.assertCanAccessQa(merchant.plan);
        if (this.useMock()) {
            return { ok: true, id };
        }
        const existing = await this.postgres.queryOne(`SELECT id::text, answer, published FROM qa_items WHERE id = $1::uuid AND business_id = $2::uuid`, [id, merchant.id]);
        if (!existing)
            throw new common_1.NotFoundException('Q&A item not found');
        ...
        if (nextPublished && !nextAnswer)
            nextPublished = false;
        await this.postgres.query(`UPDATE qa_items SET
         ...
         answered_at = CASE
           ...
           ELSE answered_at
         END
       WHERE id = $1::uuid AND business_id = $2::uuid`, [id, merchant.id, nextQuestion ?? null, nextAnswer, nextPublished]);
        if (nextPublished)
            this.pingProfileSearch(merchant.slug);
        return { ok: true, id };
    }
    async deleteQaForOwner(ownerId, id) {
        const merchant = await this.getMerchantForOwner(ownerId);
        this.planLimits.assertCanAccessQa(merchant.plan);
        if (this.useMock())
            return { ok: true };
        const result = await this.postgres.query(`DELETE FROM qa_items WHERE id = $1::uuid AND business_id = $2::uuid`, [id, merchant.id]);
        if (result.rowCount === 0) {
            throw new common_1.NotFoundException('Q&A item not found');
        }
        return { ok: true };
    }
    async listTeamMembersForOwner(ownerId) {
        const merchant = await this.getMerchantForOwner(ownerId);
        const limits = this.planLimits.limitsFor(merchant.plan);
        if (this.useMock()) {
            return {
        ...
        const members = await this.postgres.queryMany(`SELECT id::text, email, role, accepted_at::text, created_at::text
       FROM team_members WHERE business_id = $1::uuid ORDER BY created_at ASC`, [merchant.id]);
        return {
    ...
    async inviteTeamMemberForOwner(ownerId, dto) {
        const merchant = await this.getMerchantForOwner(ownerId);
        ...
        const row = await this.postgres.queryOne(`INSERT INTO team_members (business_id, email, role, invited_by)
       VALUES ($1::uuid, $2, $3, $4::uuid)
       ON CONFLICT (business_id, email) DO UPDATE SET role = EXCLUDED.role
       RETURNING id::text`, [merchant.id, email, role, ownerId]);
        return { ok: true, id: row?.id, email, role, status: 'pending' };
    }
    async removeTeamMemberForOwner(ownerId, memberId) {
        const merchant = await this.getMerchantForOwner(ownerId);
        if (this.useMock())
            return { ok: true };
        const result = await this.postgres.query(`DELETE FROM team_members WHERE id = $1::uuid AND business_id = $2::uuid`, [memberId, merchant.id]);
        if (result.rowCount === 0) {
            throw new common_1.NotFoundException('Team member not found');
        }
        return { ok: true };
    }
    async getMerchantByWhiteLabelDomain(host) {
       ...
       LIMIT 1`, [domain, domain.split('.')[0] ?? domain]);
        return row;
    }
    async listSitemapMerchants() {
        if (this.useMock()) {
            return [{ slug: MOCK_MERCHANT.slug, lastModified: new Date().toISOString() }];
        }
        const rows = await this.postgres.queryMany(`SELECT b.slug,
              GREATEST(
                b.created_at,
    ...
    async getSeoHealthForOwner(ownerId) {
        const merchant = await this.getMerchantForOwner(ownerId);
        const limits = this.planLimits.limitsFor(merchant.plan);
        if (this.useMock()) {
            return {
                ...
                checks: {
                    ...
                    faq_schema: false,
                    syndication_available: Boolean(limits.syndication),
        ...
        const qaRow = await this.postgres.queryOne(`SELECT COUNT(*)::int AS count FROM qa_items
       WHERE business_id = $1::uuid AND published = true AND answer IS NOT NULL`, [merchant.id]);
        const publishedQa = qaRow?.count ?? 0;
        const summaryFresh = merchant.review_summary_generated_at != null &&
        ...
        return {
            ...
            checks: {
                profile_indexable: (merchant.public_profile_enabled ?? true) && merchant.review_count >= 1,
                meta_filled: Boolean(merchant.seo_title?.trim() && merchant.seo_description?.trim()),
            ...
            indexnow_enabled: this.indexNow.isEnabled(),
            plan_features: {
                ai_meta_suggestions: Boolean(limits.ai_meta_suggestions),
                ai_review_summary: Boolean(limits.ai_review_summary),
                ai_qa_suggestions: Boolean(limits.ai_qa_suggestions),
            },
        };
    }
};
exports.EarnedstarService = EarnedstarService;
exports.EarnedstarService = EarnedstarService = EarnedstarService_1 = __decorate([
...
//# sourceMappingURL=earnedstar.service.js.map